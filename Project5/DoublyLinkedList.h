...
public:
	...
	DLinkedList<E>& DLinkedList<E>::operator=(DLinkedList<E>&); // assignment operator
...

{
	...
	if (obj.head != obj.tail) {
		...
		for (DNode<E>* curr = obj.head->next; curr != obj.tail; curr = curr->getNext())
		{
...
DLinkedList<E>& DLinkedList<E>::operator=(DLinkedList<E>& rhs) // assignment operator
{
	if (this != &rhs) {
		...
		for (DNode<E>* curr = rhs.head->getNext(); curr != rhs.getTail(); curr = curr->getNext())
		{
			addBack(curr->getElem());
		}//end for
	}
